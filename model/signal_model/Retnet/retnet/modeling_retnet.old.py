...
class Retention(nn.Module):
    ...
    def forward(self, q, k, v, 
        ...
        if forward_impl == 'parallel':
            ...
            """
            retention_mask = retention_mask.float().view(-1, 1, 1, 1) if retention_mask is not None else torch.ones(k.size(0), 1, 1, 1).to(decay)
            # (b, h, v_dim, qk_dim)
...
class MultiScaleRetention(nn.Module):
    ...
    def __init__(
        ...
        self.self_retention= Retention(config)
...
class RetNetDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if self.drop_path is not None:hidden_states = self.drop_path(hidden_states)

        hidden_states = self.residual_connection(hidden_states, residual)
        if not self.normalize_before:hidden_states = self.retention_layer_norm(hidden_states)
...
class RetNetModel(RetNetPreTrainedModel):
    ...
    def __init__(self, config: RetNetConfig, embed_tokens: nn.Embedding = None):
        ...
        self.layernorm_embedding = RMSNorm(self.embed_dim, eps=config.layernorm_eps) if config.layernorm_embedding else None
        ...
        self.layer_norm = RMSNorm(self.embed_dim, eps=config.layernorm_eps) if config.decoder_normalize_before else None
