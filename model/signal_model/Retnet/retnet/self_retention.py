...
try:
    ...
    pass
try:
    from .triton_kernel import parallel_retention as triton_parallel_retention
except:
    triton_parallel_retention = None
...
class SelfRetentionV1(nn.Module):
    def __init__(self,config):
        ...
        self.group_norm = groupnorm_pool[config.groupnorm_type](self.head_dim, eps=config.layernorm_eps, elementwise_affine=False)
...
class RetNetRelPosV2(nn.Module):
    ...
    def forward(self,
        ...
        if self.training and slen in self.cache_mask and recurrent_chunk_size in self.cache_mask[slen]:
            ...
            ### one well-known extreme case is that the slen too large thus below code will become inefficient
...
class SelfRetentionV2(nn.Module):
    ...
    def forward(self, q, k, v,
        ...
        if self.config.groupnorm_type != 'RMSNorm':
            assert normlize_for_stable == -3 , "only RMSNorm support automative normlize_for_stable, if you use other normlization method, please set normlize_for_stable to -3"
        ...
        kargs = {
            ...
            'only_output': forward_impl == 'parallel' and self.training and not self.config.testing_mode
        }
        if monitor_retention:
            assert mode == 'kv_first', "only support kv_first mode when monitor_retention is True, please use --retention_mode kv_first"
        if mode == 'kv_first':
            o, cache,increment =  self.kv_first_forward(q, k, v, decay_system, monitor_retention=monitor_retention, output_increment=output_increment, **kargs)
            if monitor_retention:
                retention_norm = cache.pop('retention_monitor')
            ...
            if output_increment:increment = get_output_increment(k,v,output_increment)
        elif mode == 'triton_parallel':
            assert triton_parallel_retention is not None, "we dont find the triton function, please check~~!!!!"
            o, cache = self.triton_forward(q, k, v, decay_system,**kargs)
            if output_increment:increment = get_output_increment(k,v,output_increment)
        else:
            raise NotImplementedError("mode must be 'kv_first' or 'qk_first'")
        
        return self.group_norm(o) , retention_norm, cache, increment
    ...
    def kv_first_forward(q, k, v,
        ...
        if monitor_retention:
            cache['retention_monitor'] = {}
            cache['retention_monitor']["kvflow"] = unnormlized_kv.detach().norm(dim=(-2,-1)).permute(0,2,1) #(B, H, L) ->(B,L,H) ## we will concat each layers of kv flow, thus use half precision
            #cache['retention_monitor']["numerator_norm"] = numerator.detach().norm(dim=(-1)).permute(0,2,1) #(B, H, L) ->(B,L,H)
            
            if normlize_for_stable:
                cache['retention_monitor']["gkflow"] = unnormlized_gk.detach().norm(dim=(-1)).permute(0,2,1) #(B, H, L) ->(B,L,H)
    ...
    @staticmethod
    def triton_forward(q, k, v, decay_system,
                     past_key_value=None,
                     retention_mask=None,only_output=False,
                     normlize_for_stable=True):
        """
        ...
        # (b, h, v_dim, qk_dim)
        assert past_key_value is None, "kv_reduce_fast_forward only work for the parallel mode"
        assert not normlize_for_stable, "should not use this term for triton retention"
        chunk_gamma, unnormlized_decay_mask, mask_normlizer, Coef_Q, Coef_K= decay_system
        B,H,S1,D1 = q.shape
        B,H,S2,D1 = k.shape
        B,H,S2,D2 = v.shape
        ...
        q = q/mask_normlizer.view(1, H, C1, 1)
        scale  = D1 ** -0.5
        output = triton_parallel_retention(q, k, v, chunk_gamma[:,0])/scale ### <-- triton implement will use a scale inside ### if use RMSNorm, this scale works nothing
        output = output.permute(0, 2, 1, 3)
        cache = {"unnormlized_kv": None,
                "unnormlized_gk": None ,
                "normlize_scale": None  # <-- used for check flow correct
...
if __name__ == "__main__":
    ...
    def whole_recurrent(q, k, v, retnet_rel_pos, model,use_gk=True, mode='qk_first'):
        ...
        for i in range(0,S):
            ...
            one_step_output,cache = model(q[:,:,i:i+1],k[:,:,i:i+1],v[:,:,i:i+1],
                                                                decay_system,
                                                                past_key_value= cache,
                                                                normlize_for_stable=use_gk, mode=mode)[:2]
            #print(past_kv['normlize_scale'].squeeze()[:2].cpu().numpy())
            full_rnn_state.append(one_step_output)
        full_rnn_state = torch.cat(full_rnn_state, dim=1)
    ...
    groupnorm_type = 'RMSNorm'
    config = RetNetConfig(decoder_layers=1,
                      decoder_embed_dim=256,
                      decoder_value_embed_dim=256,
                      decoder_retention_heads=8,
                      decoder_ffn_embed_dim=128,testing_mode=True, 
                      groupnorm_type=groupnorm_type)
    S = 30
    B = 2
    H = 8
    qk_dim = 32
    ...
    group_norm       = groupnorm_pool[groupnorm_type](v_dim,0,False)
