# -*- coding: utf-8 -*-
# modified from https://github.com/sustcsonglin/flash-linear-attention/blob/main/fla/layers/multiscale_retention.py 
...
def parallel_retention_fwd_kernel(
    ...
    b_b = tl.math.log2(tl.load(decay_per_head + i_h).to(tl.float32))
...
def _parallel_retention_bwd_dq(
    ...
    s_vo_t, s_vo_d, B, H, T, scale, decay_per_head,
...
def parallel_retention_bwd_kernel(
    ...
    _parallel_retention_bwd_dq(
        i_bh, i_c, i_k, i_v, i_h,
        k, v, do, dq, s_qk_h, s_qk_t, s_qk_d, s_vo_h,
        s_vo_t, s_vo_d, B, H, T, scale, decay_per_head, BTL=BTL, BTS=BTS, BK=BK, BV=BV, DK=DK, DV=DV
    )
    tl.debug_barrier()
    _parallel_retention_bwd_dkv(
        i_bh, i_c, i_k, i_v, i_h,
        q, k, v, do, dk, dv, s_qk_h, s_qk_t, s_qk_d, s_vo_h,
        s_vo_t, s_vo_d, B, H, T, scale,decay_per_head, BTL, BTS, BK, BV, DK, DV
    )
...
class ParallelRetentionFunction(torch.autograd.Function):
    ...
    @contiguous
    @custom_fwd
    def forward(ctx, q, k, v, decay_per_head):
        BTL, BTS = 128, 32
        assert BTL % BTS == 0
        ...
        decay_per_head = decay_per_head.to(v)
        #print(q.dtype, k.dtype, v.dtype, decay_per_head.dtype)
        ...
        parallel_retention_fwd_kernel[grid](
            ...
            batch_size, n_heads, seq_len, scale, decay_per_head,
            BTL=BTL, BTS=BTS, BK=BK, BV=BV, DK=d_head_qk, DV=d_head_v,
            num_warps=num_warps,
            num_stages=num_stages
        )
        ctx.save_for_backward(q, k, v, decay_per_head)
    ...
    @custom_bwd
    def backward(ctx, do):
        q, k, v, decay_per_head = ctx.saved_tensors
        BTL, BTS = 64, 32
        assert BTL % BTS == 0
        BK = min(128, triton.next_power_of_2(k.shape[-1]))
        BV = min(128, triton.next_power_of_2(v.shape[-1]))
