...
#             items.extend(retrieve_dict(v).items())
#         else:
#             items.append((new_key, v))
#     pool={}
#     for key, v in items:
#         if  key in pool:
#             assert pool[key] == v , f"key={key} conflict. One is {pool[key]}!={v}. please check your config file"
#             continue
#         pool[key] = v 
#     return dict(items)
...
def retrieve_dict(d,exclude_key=['downstream_pool']):
    ...
    for key, full_key_pool in kkmap.items():
        if len(full_key_pool)>1:
            if len(set([v for k,v in full_key_pool]))==1:
                k, v = full_key_pool[0]
                pool[key] = v 
            else:
